...
export const Main = () => {
  ...
  const pressPopupBtn = () => {
    ...
    if (currentTurn < basicLists.length - 1) {
      //0,1,2,3,4,5]
      setTimeout(() => {
        readClue(newCurrentTurn);
      }, 1200);
      setTimeout(() => {
        showClue(newCurrentTurn);
      }, 1000);
  ...
  const placeHandPc2 = (currentNum) => {
    ...
    const result1 = result.map((list) =>
      list.id === basicLists[currentTurn].id
        ? { ...list, answer: "", handPc: "", hand: "" }
        : list
    );
  ...
  const eraseLast = () => {
    ...
    const result = karutaLists.map((list) =>
      list.id === basicLists[currentTurn].id
        ? { ...list, answer: "", hand: "", handPc: "" }
        : list
    );
    const result2 = result.map((list) =>
      list.id === basicLists[currentTurn + 1].id
        ? { ...list, answer: "" }
        : list
    );
    ...
    <div>
      {screen ? (
        ...
        <>
          ...
          <ClueBox text={text} language={language} />
          ...
          {isResult ? <Result basicLists={basicLists} score={score} /> : null}
        </>
      )}
    </div>
  ); //return
